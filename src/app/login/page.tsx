  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (role === "admin") {
        ...
        // Professor Google Auth simulation
      ...
      <Card className="w-full max-w-md shadow-xl border-none">
        <CardHeader className="space-y-1 text-center">
          <div className="mx-auto w-12 h-12 bg-primary rounded-full flex items-center justify-center text-white mb-2">
            {role === "admin" ? <ShieldCheck size={24} /> : <GraduationCap size={24} />}
          </div>
          <CardTitle className="text-2xl font-headline">
            {role === "admin" ? "Admin Login" : "Professor Login"}
          </CardTitle>
          <CardDescription>
            {role === "admin" 
            ...
            <div className="grid gap-4">
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="bg-slate-50 px-2 text-muted-foreground">Or ID Login</span>
                </div>
              </div>
              <p className="text-center text-sm text-primary font-medium animate-pulse">
                "Scan your ID to log in"
              </p>
            </div>
          ) : (
